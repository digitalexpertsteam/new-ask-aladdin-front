...
export class SingleGuideComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.idGuid =  this._active.snapshot.params.guide;
    ...
    this._homeService.getSingleGuide(this.idGuid).subscribe(result => {
      ...
      this._Meta.addTags([
        { nasme: 'keywords', content: `${result.data[0].seo_keywords}` },
        ...
        { property:"og:image", content: `${result.data[0].twitter_image}`},
        { property:"og:image", content: `${result.data[0].facebook_image}`},
